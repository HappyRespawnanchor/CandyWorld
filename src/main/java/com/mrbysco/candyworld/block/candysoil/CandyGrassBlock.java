...
public class CandyGrassBlock extends Block implements BonemealableBlock {
        ...
        if (!worldIn.isClientSide) {
            ...
            if (worldIn.getLightEmission(pos.above()) < 4 && worldIn.getBlockState(pos.above()).getLightEmission(worldIn, pos.above()) > 2) {
                ...
                if (worldIn.getLightEmission(pos.above()) >= 9) {
                    ...
                    for (int i = 0; i < 3; i++) {
                        final BlockPos blockpos = pos.offset(RandomSource.create().nextInt(3) - 1, RandomSource.create().nextInt(5) - 3, RandomSource.create().nextInt(3) - 1);
    ...
    public boolean isValidBonemealTarget(LevelReader levelReader, BlockPos pos, BlockState state, boolean isClient) {
        return state.is(ModBlocks.DARK_CANDY_GRASS.get()) && state.canSustainPlant(levelReader, pos, Direction.UP, (IPlantable) ModBlocks.COTTON_CANDY_PLANT.get());
    }
